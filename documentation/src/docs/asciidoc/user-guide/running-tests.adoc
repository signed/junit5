...
the JUnit Gradle plugin will only run tests for the desired test engines. The same
applies to package names that can be included or excluded using `packages
{include ...}` or `packages {exclude ...}`. Similarly, if you supply a
_<<tag-expression>>_ via `tags {include ...}` or `tags {exclude ...}`, the JUnit Gradle
plugin will only run tests that are _tagged_ with tags matching _the expression_
(e.g., via the `@Tag` annotation for JUnit Jupiter based tests).
...
You can filter tests by _<<tag-expression>>_ using the following configuration properties.

- to specify an include tag expression, use either `groups` or `includeTags`
- to specify an exclude tag expression, use either `excludedGroups` or `excludeTags`
	...
	<build>
		<plugins>
			...
			<plugin>
				...
				<configuration>
			        <properties>
						<includeTags>fast & smoke</includeTags>
			            <excludeTags>slow, ci</excludeTags>
			        </properties>
			    </configuration>
				<dependencies>
					...
